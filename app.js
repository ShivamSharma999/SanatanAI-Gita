...
function bindEvents() {
  ...
  dom.chaptersGrid.addEventListener("click", (event) => {
  ...
  dom.verseChapterFilter.addEventListener("change", () => {
    state.selectedChapter = dom.verseChapterFilter.value;
    state.currentPage = 1;
    renderVerses();
  });

  dom.verseSearchInput.addEventListener("input", () => {
    state.verseSearch = dom.verseSearchInput.value.trim();
    state.currentPage = 1;
    renderVerses();
  });

  dom.versePageSize.addEventListener("change", () => {
...
function configureVerseAudioPlayer() {
  verseAudioPlayer.addEventListener("playing", () => {
  ...
  verseAudioPlayer.addEventListener("waiting", () => {
    if (!state.activeAudioKey) {
      return;
    }
  ...
  verseAudioPlayer.addEventListener("ended", () => {
    resetAudioState();
  });

  verseAudioPlayer.addEventListener("error", () => {
    if (!state.activeAudioKey) {
      return;
    }
